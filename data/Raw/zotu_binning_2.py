...
# # Matching Zotu's to Consensus Sequences - Temperature Experiment
...
# %%
tally_df = pd.read_csv('/Users/willsharpless/Documents/thesis/APA002/APA002.tsv', sep='\t')
...
zotu_path = "/Users/willsharpless/Documents/Thesis/APA002/APA002.fna"
...
extra_3_2_df = pd.read_pickle("extra_tp3_temperature_df.pkl")

tb_df = pd.concat([tb_df, extra_3_2_df]).groupby(['Plate','Well']).sum().reset_index()
...
def e_n_p(pair, actual_pair, 
    ...
    # erase any sample data where total reads <= 20
...
pkpa_t_data = e_n_p(pair, actual_pair, log_scale=False)

pair = 'RA - PK' # intended well pair
actual_pair = 'RA - PK' # pair were getting data for
rapk_t_data = e_n_p(pair, actual_pair, log_scale=False)

# %% Triplicates

pair = 'RA - FG' # intended well pair
actual_pair = 'RA - PK - PA' # pair were getting data for
rapkpa_t_data = e_n_p(pair, actual_pair, log_scale=False)

pair = 'SK - FG' # intended well pair
actual_pair = 'SK - PK - PA' # pair were getting data for
skpkpa_t_data = e_n_p(pair, actual_pair, log_scale=False)

pair = 'BM - FG' # intended well pair
actual_pair = 'PK - BM - PA' # pair were getting data for
pkbmpa_t_data = e_n_p(pair, actual_pair, log_scale=False)

pair = 'FG - FG' # intended well pair
actual_pair = 'PK - PA - FG' # pair were getting data for
pkpafg_t_data = e_n_p(pair, actual_pair, log_scale=False)

pair = 'MP - FG' # intended well pair
actual_pair = 'MP - PK - PA' # pair were getting data for
mppkpa_t_data = e_n_p(pair, actual_pair, log_scale=False)

# %% Full Community

pair = 'Full Community'
actual_pair = ':'
full_t_data = e_n_p(pair, actual_pair, plot_raw=True, log_scale=True)

# %% Export np arrays

dir_path = "/Users/willsharpless/Documents/ipy/Arkinlab/SynComm_Analysis" + "/RA_Data/"
ar_names = ["pkpa_t_data", "rapk_t_data", "rapkpa_t_data", "skpkpa_t_data", "pkbmpa_t_data", "pkpafg_t_data", "mppkpa_t_data", "full_t_data"]

for ix, ar in enumerate([pkpa_t_data, rapk_t_data, rapkpa_t_data, skpkpa_t_data, pkbmpa_t_data, pkpafg_t_data, mppkpa_t_data, full_t_data]):
